...
# Final Map
leaflet(RQ_MCR_SITE) %>% # add data from case
  ...
  addCircles(data = Fish_Chronic, ~LONGITUDE, ~LATITUDE, fillColor = ~mypalette(log10(SUM_Q_AVG)), fillOpacity = 0.7, color="white", radius=~sqrt(TOTAL)*50,
             stroke=FALSE, weight = 1, label = mytext, labelOptions = labelOptions( style = list("font-weight" = "normal",
  ...
  addLegend( pal=mypalette, values=~log10(SUM_Q_AVG), opacity=0.9, title = "Risk Quotient (log)", position = "bottomright", labFormat =labelFormat(between = " to "))%>%
  showGroup("Fish_Acute")
